...
BookmarkPrivate::BookmarkPrivate(const BookmarkPrivate& other)
    : QSharedData(other)
    , m_name(other.m_name)
    , m_type(other.m_type)
    , m_pidl(other.m_pidl)
    ...
    : d(new BookmarkPrivate())
{
    ...
    if (path.startsWith(QLatin1String("ftp://"), Qt::CaseInsensitive))
    {
        ...
        if (withPassword) d->m_password = folder->password();
    }
    else
    {
...
static QByteArray encrypt(const QString& value)
{
    ...
    if (!CryptProtectData(&input, L"", nullptr, nullptr, nullptr, 0, &output)) return QByteArray();
...
static QString decrypt(const QByteArray& value)
{
    ...
    if (!CryptUnprotectData(&input, nullptr, nullptr, nullptr, nullptr, 0, &output)) return QString();
