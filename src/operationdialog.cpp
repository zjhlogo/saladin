...
OperationDialog::OperationDialog(Flags flags, QWidget* parent)
    : QDialog(parent)
{
    ...
    if (flags.testFlag(WithName))
    {
        ...
        if (!flags.testFlag(CanEditName)) m_nameEdit->setReadOnly(true);
    ...
    if (flags.testFlag(WithMultiRename)) rename();

    if (row > 0) m_mainLayout->addSpacing(5);
...
void OperationDialog::setPrompt(const QString& text)
{
    ...
    if (!m_multiRename) setMaximumHeight(sizeHint().height());
}
...
void OperationDialog::setName(const QString& text)
{
    if (m_nameEdit)
    {
        ...
        if (!m_nameEdit->isReadOnly())
        {
            ...
            if (pos <= 0 || text.contains(QRegExp("[:\\\\/]"))) pos = text.length();
...
{
    if (m_locationEdit) m_locationEdit->setText(text);
}

void OperationDialog::setSource(const QString& text)
{
    if (m_sourceEdit) m_sourceEdit->setText(text);
}

void OperationDialog::setTarget(const QString& text)
{
    if (m_targetEdit) m_targetEdit->setText(text);
}
...
QStringList OperationDialog::patternList() const
{
    ...
    if (m_patternComboBox)
    {
        ...
        for (int i = 0; i < m_patternComboBox->count(); i++)
        {
            ...
            if (item != list.first() && list.count() < 10) list.append(item);
        }
    }
    return list;
}
...
void OperationDialog::setNames(const QStringList& names)
{
    ...
    if (m_multiRename) m_multiRename->setInputNames(names);
}
...
void OperationDialog::expandTo(const QSize& newSize)
{
    if (isVisible())
    {
        ...
        if (rect.bottom() > screen.bottom()) rect.moveBottom(screen.bottom());
        if (rect.top() < screen.top()) rect.moveTop(screen.top());

        if (rect.right() > screen.right()) rect.moveRight(screen.right());
        if (rect.left() < screen.left()) rect.moveLeft(screen.left());
