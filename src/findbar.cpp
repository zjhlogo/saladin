...
{
    if (m_boundWidget && !e->spontaneous()) m_boundWidget->setFocus();
}
...
bool FindBar::eventFilter(QObject* obj, QEvent* e)
{
    ...
    if (obj == m_comboBox && e->type() == QEvent::KeyPress)
    {
        ...
        if (m_boundWidget && (ke->key() == Qt::Key_Up || ke->key() == Qt::Key_Down || ke->key() == Qt::Key_PageUp || ke->key() == Qt::Key_PageDown))
        {
...
{
    if (m_enabled) emit find(m_comboBox->currentText());
}
