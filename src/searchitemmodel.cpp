...
SearchItemModel::SearchItemModel(QObject* parent)
...
{
    if (m_pendingFolders.isEmpty()) return;
...
bool SearchItemModel::scanFolder(const QString& prefix, ShellFolder* folder)
{
    ...
    if (foundItems.isEmpty()) return false;
...
{
    if (!parent.isValid()) return m_items.count();
    return 0;
}
...
QVariant SearchItemModel::data(const QModelIndex& index, int role /*= Qt::DisplayRole*/) const
{
    if (role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        if (row >= 0)
        {
            ...
            case Column_Size:
                if (item.state().testFlag(ShellItem::HasProperties)) return QLocale::system().toString(item.size()) + ' ';
                break;
            case Column_LastModified:
    ...
    {
        int row = index.row();
        if (row >= 0) return m_items.at(row).m_folder->toolTip(m_items.at(row).m_item);
    }

    if (role == Qt::TextAlignmentRole && index.column() == Column_Size) return (int)(Qt::AlignRight | Qt::AlignVCenter);
