...
{
    if (e->type() != QEvent::MouseButtonPress && e->type() != QEvent::MouseButtonDblClick && e->type() != QEvent::FocusIn && e->type() != QEvent::ContextMenu)
        return false;
    ...
    if (((widget->windowFlags().testFlag(Qt::Dialog)) && widget->isModal()) || (widget->windowFlags().testFlag(Qt::Popup))
        || (widget->windowFlags().testFlag(Qt::Tool)))
        return false;

    while (widget)
    {
        if (widget->topLevelWidget() != this) return false;
...
void MainWindow::changeEvent(QEvent* e)
{
    ...
    if (e->type() == QEvent::ThemeChange) QTimer::singleShot(100, application, SLOT(updateTheme()));
}

void MainWindow::setSourcePane(int index)
{
    if (m_sourcePane == m_panes[index]) return;
...
void MainWindow::viewCurrent()
{
    ...
    if (!item.isValid() || !item.attributes().testFlag(ShellItem::Stream)) return;
...
void MainWindow::viewSelected()
{
    ...
    {
        if (item.isValid() && item.attributes().testFlag(ShellItem::Stream)) items.append(item);
    }
...
void MainWindow::editNew()
{
    ...
    if (!folder->createFile(name, false)) return;
...
void MainWindow::transferSelection(ShellSelection* selection, ShellFolder* targetFolder, ShellSelection::TransferType type, bool canRename)
{
    ...
    if (!canTransfer && !canDragDrop) return;
    ...
    if (canTransfer) dialog.setCheckBoxText(tr("O&verwrite without prompt"));
    ...
    if (canTransfer)
    {
        ...
        if (dialog.checkBoxChecked()) transferFlags |= ShellSelection::ForceOverwrite;
    ...
    if (done) m_sourcePane->unselectAll();

    if (done && sameTarget && items.count() == 1) m_sourcePane->setGotoItemName(dialog.name());
}
...
void MainWindow::createFolder()
{
    ...
    if (done) m_sourcePane->setGotoItemName(name);
}
...
void MainWindow::packToZip()
{
    ...
    if (items.count() == 1)
    {
        ...
        if (pos > 0) itemName.truncate(pos);
    ...
    if (!targetFolder->createFile(name, dialog.checkBoxChecked(), zipData, sizeof(zipData))) return;
...
void MainWindow::createToolStrip()
{
    ...
    strip->setStyleSheet("#ToolStrip { border: 0px }");
