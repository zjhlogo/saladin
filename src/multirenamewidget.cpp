...
MultiRenameWidget::MultiRenameWidget(QWidget* parent)
    : QWidget(parent)
{
    ...
    for (QChar ch = *data++; ch != 0; ch = *data++)
    {
        if (ch == QLatin1Char('['))
        {
            ...
            if (op == QLatin1Char('n') || op == QLatin1Char('e'))
            {
                int from = 1;
                int to = -1;
                if (*opdata == ':') from = to = parseInt(opdata + 1, &opdata, 1);
                if (opdata && *opdata == ':') to = parseInt(opdata + 1, &opdata, -1);
                if (opdata && *opdata == ']')
                {
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        if (pos < length && n > 0) result[i].append(part.mid(pos, n));
                    }
                    data = opdata + 1;
                    continue;
                }
            ...
            if (op == QLatin1Char('c'))
            {
                ...
                int from = 1;
                int step = 1;
                if (*opdata == ':') width = parseInt(opdata + 1, &opdata, 1);
                if (opdata && *opdata == ':') from = parseInt(opdata + 1, &opdata, 1);
                if (opdata && *opdata == ':') step = parseInt(opdata + 1, &opdata, 1);
                if (opdata && *opdata == ']')
                {
                    for (int i = 0; i < count; i++)
                    {
