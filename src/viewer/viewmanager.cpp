...
void ViewManager::updateTitle(ViewItem& item, const QString& name)
{
    ...
    if (title.isEmpty()) title = tr("Unknown file");

    if (item.m_pidls.count() > 1) title += QString(" - ") + tr("%1 of %2").arg(item.m_index + 1).arg(item.m_pidls.count());
...
bool ViewManager::checkType(const ShellPidl& pidl, View::Type inType, View::Type& outType, QByteArray& format, QString& name)
{
    ...
    if (inType == View::Binary) return true;

    if (inType == View::Auto || inType == View::Image)
    {
        outType = View::Image;

        if (FormatHelper::checkImage(file, inType == View::Image, format)) return true;
    }
    ...
    if (inType == View::Auto || inType == View::Text)
    {
        ...
        if (FormatHelper::checkText(file, inType == View::Text, format)) return true;
