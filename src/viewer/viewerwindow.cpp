...
ViewerWindow::~ViewerWindow()
{
    if (!(windowState() & Qt::WindowFullScreen)) storeGeometry(false);
}
...
void ViewerWindow::setView(View* view)
{
    ...
    if (windowState() & Qt::WindowFullScreen) view->setFullScreen(true);
...
{
    if (!e->spontaneous() && !(windowState() & Qt::WindowFullScreen)) storeGeometry(true);
}
...
void ViewerWindow::initializeGeometry()
{
    ...
    if (settings->contains("ViewerWindowGeometry"))
    {
        ...
        if (settings->value("ViewerWindowOffset").toBool())
        {
            ...
            if (position.x() + frame.width() > available.right()) position.rx() = available.left();
            if (position.y() + frame.height() > available.bottom() - 20) position.ry() = available.top();
            move(position);
        }
    }
    else
