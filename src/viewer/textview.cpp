...
TextView::TextView(QObject* parent, QWidget* parentWidget)
    : View(parent)
    , m_loader(nullptr)
    , m_length(0)
...
QMenu* TextView::createEncodingMenu()
{
    ...
    for (int i = 0; i < sizeof(stdEncodings) / sizeof(stdEncodings[0]); i++)
    {
        ...
        if (stdEncodings[i].m_key != 0) action->setShortcut(stdEncodings[i].m_key);
...
void TextView::load()
{
    ...
    {
        if (action->isCheckable()) action->setChecked(action == current);
        if (action == current) m_encoding = action->text();
    }
...
void TextView::loadNextBlock()
{
    ...
    if (atEnd)
        setStatus(tr("Text") + ", " + m_encoding + " ("
                  + tr("%1 characters, %2 lines").arg(QLocale::system().toString(m_length), QLocale::system().toString(m_edit->document()->blockCount()))
                  + ")");
    else if (estimatedLength > 0)
        setStatus(tr("Text") + ", " + m_encoding + " (" + tr("loading... %1%").arg((int)(m_length * 100 / estimatedLength)) + ")");

    if (runTimer) QTimer::singleShot(0, this, SLOT(loadNextBlock()));
}
...
void TextView::contextMenuRequested(const QPoint& pos)
{
    ...
    if (menu) menu->popup(m_edit->mapToGlobal(pos));
}
...
void TextView::setFullScreen(bool on)
{
    ...
    if (on)
    {
        ...
        if (QFrame* frame = qobject_cast<QFrame*>(layout->itemAt(0)->widget())) frame->setFrameStyle(0);
    }
    else
    {
        layout->setContentsMargins(3, 0, 3, 0);
        if (QFrame* frame = qobject_cast<QFrame*>(layout->itemAt(0)->widget())) frame->setFrameStyle(QFrame::StyledPanel | QFrame::Sunken);
    }
}
