    ...
    : QDialog(parent)
{
    ...
    connect(m_list->selectionModel(), SIGNAL(selectionChanged(const QItemSelection&, const QItemSelection&)), this, SLOT(updateActions()));
...
void BookmarksDialog::listContextMenu(const QPoint& pos)
{
    ...
    if (index.isValid())
    {
        ...
        if (menu) menu->exec(m_list->viewport()->mapToGlobal(pos));
    }
}
